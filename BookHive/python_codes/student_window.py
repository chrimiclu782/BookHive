...
class StudentWindow(QWidget):
    ...
    def borrow_book(self):
        ...
        try:
            ...
            # Check latest borrow status
            cursor.execute("""
                SELECT borrow_status FROM Borrowed 
                WHERE book_id = %s 
                ORDER BY date_borrowed DESC LIMIT 1
            """, (book_id,))
            status_row = cursor.fetchone()
            if status_row and status_row[0] == 'Borrowed':
                QMessageBox.warning(self, "Unavailable", "This book is currently borrowed by another student.")
                conn.close()
                return
