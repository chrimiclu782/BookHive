...
from book_history_window import BookHistoryWindow
...
class LibrarianWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.search_input.setPlaceholderText("Search by title or book ID")
    ...
    def search_books(self):
        ...
        Performs a partial match search and displays matching results.
        ...
        conn = connect_db()
        cursor = conn.cursor()
        cursor.execute("SELECT book_id, title, author, genre, location, supplier_id, price FROM Books WHERE title LIKE %s OR book_id LIKE %s ORDER BY book_id", (f"%{keyword}%", f"%{keyword}%"))
        books = cursor.fetchall()
        conn.close()
        if books:
            table_html = """
