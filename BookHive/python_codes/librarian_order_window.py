...
from PyQt5.QtWidgets import (
    QWidget, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QMessageBox, QScrollArea, QComboBox, QCheckBox, QGroupBox, QLineEdit, QApplication
)
...
class LibrarianOrderWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.setGeometry((screen.width() - 800) // 2, 50, 800, 600)
    ...
    def filter_orders(self):
        ...
        for checkbox, order_id, _ in self.order_checkboxes:
            # Extract order_id from the checkbox text (format: "Order {order_id} - ...")
            text = checkbox.text()
            if text.startswith("Order "):
                order_id_str = text.split(" - ")[0].replace("Order ", "")
                try:
                    oid = int(order_id_str)
                    if search_text and str(oid).startswith(search_text):
                        checkbox.show()
                    ...
                    checkbox.hide()  # Hide if parsing fails
            else:
                checkbox.hide()
